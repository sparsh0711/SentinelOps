...
def _projection(source):
    ...
    $result = foreach ($event in $events) {{
      ...
      [pscustomobject]@{{
        ...
        destination_ip = if ($data.DestinationIp) {{ $data.DestinationIp }} elseif ($data.DestinationIpAddress) {{ $data.DestinationIpAddress }} else {{ '' }}
        destination_port = if ($data.DestinationPort) {{ $data.DestinationPort }} elseif ($data.DestPort) {{ $data.DestPort }} else {{ '' }}
        process = if ($data.NewProcessName) {{ $data.NewProcessName }} elseif ($data.Image) {{ $data.Image }} else {{ '' }}
        parent_process = if ($data.ParentImage) {{ $data.ParentImage }} elseif ($data.ParentProcessName) {{ $data.ParentProcessName }} else {{ '' }}
        ...
        logon_type = if ($data.LogonType) {{ $data.LogonType }} else {{ '' }}
        target_filename = if ($data.TargetFilename) {{ $data.TargetFilename }} else {{ '' }}
        registry_key = if ($data.TargetObject) {{ $data.TargetObject }} elseif ($data.ObjectName) {{ $data.ObjectName }} else {{ '' }}
        service_name = if ($data.ServiceName) {{ $data.ServiceName }} else {{ '' }}
        hash = if ($data.Hashes) {{ $data.Hashes }} elseif ($data.Hash) {{ $data.Hash }} else {{ '' }}
