...
class Api:
    def __init__(self,settings,database,rules,summary_engine=None):self.settings=settings;self.database=database;self.rules=rules;self.summary_engine=summary_engine
    def get(self,path,query):
        ...
        if path=="/api/v2/hunts":return {"hunts":HUNTS,"runs":self.database.list_hunt_runs()},200
        if path=="/api/v2/iocs":return {"iocs":self.database.list_iocs()},200
    ...
    def post_json(self,path,payload):
        if path=="/api/v2/analyses":
            analysis=validate_analysis(payload);return {"saved":True,"id":self.database.save_analysis(analysis)},201
        if path=="/api/v2/hunts/run":
            result=run_hunt(payload.get("huntId"),payload.get("events"));result["runId"]=self.database.save_hunt_run(result,str(payload.get("sourceName") or "Unnamed source")[:255]);return result,201
        if path=="/api/v2/iocs":return self.database.save_ioc(normalize_ioc(payload)),201
        if path=="/api/v2/iocs/delete":
            ...
            self.database.delete_ioc(ioc_id);return {"deleted":True,"id":ioc_id},200
        if path=="/api/v2/iocs/match":return match_iocs(payload.get("events"),self.database.list_iocs(enabled_only=True)),200
        ...
        if path.startswith("/api/v2/incidents/") and path.endswith("/update"):
            incident_id=self._extract_nested_id(path,"incidents","update");return self.database.update_incident(incident_id,validate_incident_update(payload)),200
        if path.startswith("/api/v2/incidents/") and path.endswith("/notes"):
            incident_id=self._extract_nested_id(path,"incidents","notes");return self.database.add_incident_note(incident_id,validate_incident_note(payload)),200
